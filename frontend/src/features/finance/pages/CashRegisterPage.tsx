...
function StatCard({ icon: Icon, label, value, color, bg }: {
    ...
    return (
        <div className="bg-white rounded-xl border border-slate-200 p-3 sm:p-4 shadow-sm flex items-center gap-3 sm:gap-4 min-w-0 overflow-hidden">
            <div className={cn('w-10 h-10 sm:w-11 sm:h-11 rounded-xl flex items-center justify-center shrink-0', bg)}>
            ...
            <div className="min-w-0 flex-1">
                ...
                <p className="text-base sm:text-lg lg:text-xl font-black tabular-nums text-slate-900 tracking-tight break-words leading-tight">{value}</p>
            </div>
        </div>
    );
}
                        ...
                        return (
                            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                    <div className="bg-slate-50 rounded-xl p-3 border border-slate-100 text-center min-w-0 overflow-hidden">
                                        <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">COP</p>
                                        <p className="text-sm sm:text-base font-black text-slate-800 tabular-nums break-words leading-tight">{formatCurrency(breakdown.COP)}</p>
                                    </div>
                                    <div className="bg-blue-50 rounded-xl p-3 border border-blue-100 text-center min-w-0 overflow-hidden">
                                        <p className="text-[10px] font-bold text-blue-400 uppercase mb-1">USD</p>
                                        <p className="text-sm sm:text-base font-black text-blue-800 tabular-nums break-words leading-tight">${breakdown.USD.toFixed(2)}</p>
                                    </div>
                                    <div className="bg-amber-50 rounded-xl p-3 border border-amber-100 text-center min-w-0 overflow-hidden">
                                        <p className="text-[10px] font-bold text-amber-400 uppercase mb-1">VES</p>
                                        <p className="text-sm sm:text-base font-black text-amber-800 tabular-nums break-words leading-tight">Bs. {breakdown.VES.toFixed(2)}</p>
                                    </div>
                                </div>
                            </div>
                        );
                        ...
                        {transactions.length === 0 ? (
                            ...
                            <div className="divide-y divide-slate-100">
                                {transactions.map((t: any) => (
                                    <div key={t.id} className={cn("px-3 sm:px-5 py-3 flex items-center justify-between gap-2 hover:bg-slate-50 transition-colors group", t.type === 'SALE' && "cursor-pointer")} onClick={() => t.type === 'SALE' && setSelectedSaleId(t.id)}>
                                        <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                                            ...
                                            <div className="min-w-0">
                                                ...
                                                <p className="text-xs text-slate-400 flex items-center gap-1 mt-0.5">
                                                    ...
                                                    <span className="truncate">{new Date(t.createdAt).toLocaleDateString('es-VE')} {new Date(t.createdAt).toLocaleTimeString('es-VE', { hour: '2-digit', minute: '2-digit' })}</span>
                                                </p>
                                                {t.notes && (
                                                    <p className="text-[10px] bg-slate-100/80 text-slate-600 px-2 py-0.5 rounded font-semibold mt-1 border border-slate-200/50 w-fit max-w-full truncate">
                                                        {t.notes}
                                                    </p>
                                                )}
                                            </div>
        ...
        <div className="min-h-full">
            <div className="bg-white border-b border-slate-200 mb-6">
                <div className="px-4 sm:px-6 py-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-lg w-fit shrink-0 overflow-x-auto max-w-full">
                        <button onClick={() => setActiveTab('current')} className={cn('flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 rounded-md text-xs sm:text-sm font-medium transition-colors whitespace-nowrap', activeTab === 'current' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700')}>
                            <Circle className="w-4 h-4" />Caja Actual
                        </button>
                        <button onClick={() => { setActiveTab('history'); setHistoryPage(1); }} className={cn('flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 rounded-md text-xs sm:text-sm font-medium transition-colors whitespace-nowrap', activeTab === 'history' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700')}>
                            <History className="w-4 h-4" />Historial
                        </button>
                        <button onClick={() => setActiveTab('egresos')} className={cn('flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 rounded-md text-xs sm:text-sm font-medium transition-colors whitespace-nowrap', activeTab === 'egresos' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500 hover:text-slate-700')}>
