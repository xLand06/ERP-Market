# Change Spec: deploy-auto-improve

## Intent

This change hardens the multi-tenant ERP-Market VPS onboarding kit so that adding
a new client via `deploy/scripts/add-client.sh <slug>` is **fully automatic and
error-free**. It fixes five ground-truth provisioning bugs in the deploy kit
(an unassigned `CLIENT_SLUG` env var that leaves the compose `name:` empty, an
invalid `-q` flag on the Compose v5.5.1 `up` command, a wrong API build context,
a DB healthcheck missing `start_period`, and a TLS key owned by root that uid-70
Postgres cannot read), adds a deterministic idempotent rerun behavior for
existing slugs, robust tolerance for slow first boot, and a self-checking smoke
step that aborts loudly with an actionable message on failure instead of leaving
a silently broken stack. The fixes must land in the repo working tree and then
be propagated to the live VPS clone (`/opt/erp-market`) in the apply phase so a
fresh VPS clone works end-to-end with no manual steps. This is a bugfix +
hardening change only — it adds no product features.
...
## Propagation requirement (apply phase)

All fixes MUST land in the repo working tree (commit-ready) AND MUST be
propagated to the live VPS clone at `/opt/erp-market` so a fresh VPS clone works
end-to-end. The apply/verify phases MUST re-check each bug fix against the
VPS state and confirm all five bugs are gone there.
