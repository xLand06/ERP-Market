...
- [ ] Per-client Postgres TLS: the backend's cloud client hardcodes `ssl: {rejectUnauthorized:false}`, so each client DB runs `ssl=on` with a self-signed keypair generated by `add-client.sh` (in `deploy/clients/<slug>/tls/`, gitignored). The key is mode 644 so the postgres container (uid 70) can read it — it only protects the private per-client bridge; DB access is still gated by `POSTGRES_PASSWORD`.
