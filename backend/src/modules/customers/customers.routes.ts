...
router.post('/', roleGuard('MANAGER'), validate(createCustomerSchema), ctrl.createCustomer);

/** PUT  /api/customers/:id — Actualizar cliente (SELLER y superiores) */
router.put('/:id', roleGuard('MANAGER'), validate(idParamSchema, { source: 'params' }), validate(updateCustomerSchema), ctrl.updateCustomer);
